      ...
      >
        <div class="flex w-full gap-2 overflow-hidden rounded-lg bg-bg-raised shadow-xl">
          ...
          >
            ...
            >
              <IconAlertTriangle v-if="item.type === 'warning'" class="h-6 w-6" />
              <IconCircleCheck v-else-if="item.type === 'success'" class="h-6 w-6" />
              <IconCircleX v-else-if="item.type === 'error'" class="h-6 w-6" />
              <IconInfoCircle v-else class="h-6 w-6" />
            </div>
            ...
            <div class="flex items-center gap-1">
              ...
              <ButtonStyled circular size="small">
                <button v-tooltip="'Copy to clipboard'" @click="copyToClipboard(item)">
                  <IconCheck v-if="copied[createNotifText(item)]" />
                  <IconCopy v-else />
                </button>
              </ButtonStyled>
              <ButtonStyled circular size="small">
...
<script setup>
import ButtonStyled from "../modrinth/ui/base/ButtonStyled.vue";
import { IconCircleX, IconCircleCheck, IconCheck, IconInfoCircle, IconAlertTriangle, IconX, IconCopy } from "@tabler/icons-vue";
