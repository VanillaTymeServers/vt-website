<template>
    <div class="card moderation-checklist">
        ...
        <div v-else-if="generatedMessage">
            <p>
                Enter your moderation message here. Remember to check the
                Moderation tab to answer any questions an author might have!
            </p>
        ...
        <div v-else-if="steps[currentStepIndex].id === 'modpack-permissions'">
            ...
            <div v-else>
                <div v-if="modPackData[modPackIndex].type === 'unknown'">
                    ...
                    <div class="input-group">
                        <button
                            ...
                            :class="{
                                'option-selected':
                                    modPackData[modPackIndex].status ===
                                    option.id,
                            }"
                            @click="
                                modPackData[modPackIndex].status = option.id
                            "
                        >
                            {{ option.name }}
                        </button>
                    ...
                    <template
                        v-if="
                            modPackData[modPackIndex].status !== 'unidentified'
                        "
                    >
                ...
                <div v-else-if="modPackData[modPackIndex].type === 'flame'">
                    <p>
                        What is the approval type of
                        {{ modPackData[modPackIndex].title }} (<a
                ...
                >
                    <p
                        ...
                        "
                    >
                        Does this project provide identification and permission
                        for
                        <strong>{{
                            modPackData[modPackIndex].file_name
                        }}</strong
                    ...
                    <div class="input-group">
                        <button
                            ...
                            :class="{
                                'option-selected':
                                    modPackData[modPackIndex].approved ===
                                    option.id,
                            }"
                            @click="
                                modPackData[modPackIndex].approved = option.id
                            "
                        >
                            {{ option.name }}
                        </button>
        ...
        <div v-else>
            ...
            <template
                v-if="
                    steps[currentStepIndex].rules &&
                    steps[currentStepIndex].rules.length > 0
                "
            >
            ...
            <template
                v-if="
                    steps[currentStepIndex].examples &&
                    steps[currentStepIndex].examples.length > 0
                "
            >
                <strong>Examples of what to reject:</strong>
                <ul>
                    <li
                        v-for="(example, index) in steps[currentStepIndex]
                            .examples"
                        :key="index"
                    >
                        {{ example }}
            ...
            <template
                v-if="
                    steps[currentStepIndex].exceptions &&
                    steps[currentStepIndex].exceptions.length > 0
                "
            >
                <strong>Exceptions:</strong>
                <ul>
                    <li
                        v-for="(exception, index) in steps[currentStepIndex]
                            .exceptions"
                        :key="index"
                    >
                        {{ exception }}
            ...
            <p v-if="steps[currentStepIndex].id === 'links'">
                <template v-if="project.issues_url">
                    ...
                    <a class="text-link" :href="project.issues_url">{{
                        project.issues_url
                    }}</a>
                    <br />
                </template>
                <template v-if="project.source_url">
                    ...
                    <a class="text-link" :href="project.source_url">{{
                        project.source_url
                    }}</a>
                    <br />
                </template>
                <template v-if="project.wiki_url">
                    <strong>Wiki: </strong>
                    <a class="text-link" :href="project.wiki_url">{{
                        project.wiki_url
                    }}</a>
                    <br />
                </template>
                <template v-if="project.discord_url">
                    ...
                    <a class="text-link" :href="project.discord_url">{{
                        project.discord_url
                    }}</a>
                    <br />
                </template>
                ...
                >
                    ...
                    <a class="text-link" :href="donation.url">{{
                        donation.url
                    }}</a>
                    <br />
                </template>
                ...
                <button
                    ...
                    :class="{
                        'option-selected':
                            ...
                            selectedOptions[steps[currentStepIndex].id].find(
            ...
            >
                <div
                    ...
                    ].filter((x) => x.fillers && x.fillers.length > 0)"
                    :key="index"
                >
                    <div v-for="(filler, idx) in option.fillers" :key="idx">
                        <label :for="filler.id">
                            <span class="label__title">
                                {{ filler.question }}
                                <span v-if="filler.required" class="required"
                                    >*</span
                                >
                            </span>
                        </label>
            ...
            <template v-if="generatedMessage && !done">
                ...
                <div class="joined-buttons">
                    ...
                    >
                        ...
                        <template #withhold>
                            <EyeOffIcon aria-hidden="true" /> Withhold
                        </template>
                    </OverflowMenu>
                </div>
            </template>
...
Currently, your %LINK% link is inaccessible!`,
                    fillers: [
                        {
                            id: "LINK",
                            question:
                                "Please specify the link type that is inaccessible.",
                        },
                    ],
                },
            ],
        },
        {
            ...
            shown:
                ...
                props.project.additional_categories.length > 0,
            options: [
                {
                    name: "Inaccurate",
                    ...
                    fillers: [
                        {
                            id: "ORIGINAL_PROJECT",
                            question:
                                "What is the title of the original project?",
                            required: true,
                        },
                        {
                            id: "ORIGINAL_AUTHOR",
                            question:
                                "What is the author of the original project?",
                            required: true,
                        },
                    ],
                },
                    ...
                    fillers: [
                        {
                            id: "MESSAGE",
                            question:
                                "Please explain to the user how it infringes on our content rules.",
                            large: true,
                        },
                    ],
                },
...
async function initializeModPackData() {
    startLoading();
    try {
        const raw = await useBaseFetch(
            `moderation/project/${props.project.id}`,
            { internal: true },
        );
        const projects = [];
...
async function generateMessage() {
    ...
    currentStepIndex.value += 1;
    await navigateTo(
        `/${props.project.project_type}/${props.project.slug}/moderation`,
    );
}
