<template>
    <div class="mb-3 flex flex-col gap-3">
        <div class="flex flex-wrap gap-3">
            <InventoryFilterControl
                ref="itemFilters"
                :inventory="inventory"
                :base-id="`${baseId}-filter`"
                @update:query="updateQuery"
                class="flex-grow"
            />
            <ButtonStyled
                v-if="currentMember && currentMember.permissions.CREATE_ITEMS"
                :color="createItemButtonSecondary ? 'standard' : 'blue'"
            >
                <button @click="openModal"><IconPlus /> Create item</button>
            </ButtonStyled>

            <!-- <Pagination
                v-if="!openModal"
                :page="currentPage"
                class="mt-auto"
                :count="Math.ceil(inventory.length / pageSize)"
                @switch-page="switchPage"
            /> -->
        </div>

        <div
            v-if="openModal && filteredInventory.length > pageSize"
            class="flex flex-wrap items-center justify-between gap-2"
        >
            <span>
                Showing {{ (currentPage - 1) * pageSize + 1 }} to
                {{ Math.min(currentPage * pageSize, filteredInventory.length) }}
                of
                {{ filteredInventory.length }}
            </span>

            <Pagination
                :page="currentPage"
                class="mt-auto"
                :count="Math.ceil(filteredInventory.length / pageSize)"
                @switch-page="switchPage"
            />
        </div>
    </div>
    <div
        v-if="filteredInventory.length > 0"
        class="no-environment flex flex-col gap-4 rounded-2xl bg-bg-raised px-6 pb-8 pt-4 supports-[grid-template-columns:subgrid]:grid supports-[grid-template-columns:subgrid]:grid-cols-[1fr_min-content] sm:px-8 supports-[grid-template-columns:subgrid]:sm:grid-cols-[min-content_auto_auto_auto_min-content] supports-[grid-template-columns:subgrid]:xl:grid-cols-[min-content_auto_auto_auto_auto_auto_min-content]"
    >
        <div class="versions-grid-row">
            <div class="w-9 max-sm:hidden"></div>
            <div class="text-sm font-bold text-contrast max-sm:hidden">
                Name
            </div>
            <div
                class="text-sm font-bold text-contrast max-sm:hidden sm:max-xl:collapse sm:max-xl:hidden"
            >
                Price
            </div>
            <div
                class="text-sm font-bold text-contrast max-sm:hidden sm:max-xl:collapse sm:max-xl:hidden"
            >
                Status
            </div>
            <div
                class="text-sm font-bold text-contrast max-sm:hidden sm:max-xl:collapse sm:max-xl:hidden"
            >
                Created
            </div>
            <div
                class="text-sm font-bold text-contrast max-sm:hidden sm:max-xl:collapse sm:max-xl:hidden"
            >
                Availability
            </div>
            <div
                class="text-sm font-bold text-contrast max-sm:hidden xl:collapse xl:hidden"
            >
                Compatibility
            </div>
            <div
                class="text-sm font-bold text-contrast max-sm:hidden xl:collapse xl:hidden"
            >
                Stats
            </div>
            <div class="w-9 max-sm:hidden"></div>
        </div>
        <template v-for="(item, index) in currentInventory" :key="index">
            <!-- Row divider -->
            <div
                class="versions-grid-row h-px w-full bg-surface-5"
                :class="{
                    'max-sm:!hidden': index === 0,
                }"
            ></div>
            <div class="versions-grid-row group relative">
                <AutoLink
                    class="absolute inset-[calc(-1rem-2px)_-2rem] before:absolute before:inset-0 before:transition-all before:content-[''] hover:before:backdrop-brightness-110"
                    :to="`/${project.project_type}/${project.slug ? project.slug : project.id}/inventory/${item.id}`"
                />
                <div class="flex flex-col justify-center gap-2 sm:contents">
                    <div class="flex flex-row items-center gap-2 sm:contents">
                        <div class="self-center">
                            <div class="relative z-[1] cursor-pointer">
                                <Avatar
                                    :src="`data:image/png;base64,${item.product.item.icon}`"
                                    :alt="item.product.item.label"
                                    size="xs"
                                    no-shadow
                                    loading="lazy"
                                />
                            </div>
                        </div>
                        <div
                            class="pointer-events-none relative z-[1] flex flex-col justify-center"
                            :class="{
                                'group-hover:underline': !!itemLink,
                            }"
                        >
                            <div class="font-bold text-contrast">
                                {{
                                    item.product.custom_name ||
                                    item.product.item.label
                                }}
                            </div>
                            <div
                                v-if="item.product.custom_name"
                                class="text-xs font-medium"
                            >
                                {{ item.product.item.label }}
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-col justify-center gap-2 sm:contents">
                        <div
                            class="flex flex-row flex-wrap items-center gap-1 xl:contents"
                        >
                            <div class="flex items-center">
                                <div
                                    class="flex items-center justify-center gap-1"
                                    v-tooltip="
                                        `${item.price.amount} ${item.price.currency.label}${item.price.amount > 1 ? 's' : ''}`
                                    "
                                >
                                    <img
                                        :src="`data:image/png;base64,${item.price.currency.icon}`"
                                        :alt="item.price.currency.label"
                                        class="block h-5 w-5"
                                    />
                                    <span class="text-sm font-semibold">{{
                                        item.price.amount
                                    }}</span>
                                </div>
                            </div>
                            <div class="flex items-center">
                                <div class="flex flex-wrap gap-1">
                                    <TagItem
                                        class="z-[1]"
                                        :style="`--_color: ${item.status === 'active' ? 'var(--color-green)' : item.status === 'unlisted' ? 'var(--color-gray)' : item.status === 'draft' ? 'var(--color-blue)' : item.status === 'archived' ? 'var(--color-orange)' : null}`"
                                    >
                                        {{ capitalizeString(item.status) }}
                                    </TagItem>
                                </div>
                            </div>
                            <!-- <div
                                v-if="hasMultipleEnvironments"
                                class="flex items-center"
                            >
                                <div class="flex flex-wrap gap-1">
                                    <TagItem
                                        v-for="(
                                            tag, tagIdx
                                        ) in getEnvironmentTags(
                                            version.environment,
                                        )"
                                        :key="`env-tag-${tagIdx}`"
                                        class="z-[1] text-center"
                                    >
                                        <component :is="tag.icon" />
                                        {{ formatMessage(tag.label) }}
                                    </TagItem>
                                </div>
                            </div> -->
                        </div>
                        <div
                            class="flex flex-col justify-center gap-1 max-sm:flex-row max-sm:justify-start max-sm:gap-3 xl:contents"
                        >
                            <div
                                v-tooltip="
                                    $dayjs(item.created).format(
                                        'MMMM D, YYYY [at] h:mm A',
                                    )
                                "
                                class="z-[1] flex cursor-help items-center gap-1 text-nowrap font-medium xl:self-center"
                            >
                                <IconCalendar class="xl:hidden" />
                                {{ formatRelativeTime(item.created) }}
                            </div>
                            <div class="flex items-center">
                                <div class="flex flex-wrap gap-1">
                                    <TagItem
                                        class="z-[1]"
                                        :style="`--_color: ${item.availability === 'in_stock' ? 'var(--color-green)' : item.availability === 'out_of_stock' ? 'var(--color-red)' : item.availability === 'limited_stock' ? 'var(--color-blue)' : item.availability === 'restocked' ? 'var(--color-orange)' : null}`"
                                    >
                                        {{
                                            formatAvailability(
                                                item.availability,
                                            )
                                        }}
                                    </TagItem>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div
                    class="z-[1] flex items-start justify-end gap-1 max-[400px]:flex-col max-[400px]:justify-start sm:items-center"
                >
                    <slot name="actions" :inventory="item"></slot>
                </div>
                <!-- <div
                    v-if="showFiles"
                    class="tag-list pointer-events-none relative z-[1] col-span-full"
                >
                    <div
                        v-for="(file, fileIdx) in version.files"
                        :key="`platform-tag-${fileIdx}`"
                        :class="`flex items-center gap-1 text-wrap rounded-full bg-button-bg px-2 py-0.5 text-xs font-medium ${file.primary || fileIdx === 0 ? 'bg-brand-highlight text-contrast' : 'text-primary'}`"
                    >
                        <StarIcon
                            v-if="file.primary || fileIdx === 0"
                            class="shrink-0"
                        />
                        {{ file.filename }} - {{ formatBytes(file.size) }}
                    </div>
                </div> -->
            </div>
        </template>
    </div>
    <div class="mt-3 flex">
        <Pagination
            :page="currentPage"
            class="ml-auto"
            :count="Math.ceil(filteredInventory.length / pageSize)"
            @switch-page="switchPage"
        />
    </div>
</template>

<script lang="ts" setup>
import { type Ref, ref, computed } from "vue";
import { useRouter, useRoute } from "vue-router";
import Pagination from "~/components/modrinth/ui/base/Pagination.vue";
import AutoLink from "~/components/modrinth/ui/base/AutoLink.vue";
import Avatar from "~/components/modrinth/ui/base/Avatar.vue";
import Badge from "~/components/modrinth/ui/base/Badge.vue";
import InventoryTags from "~/components/modrinth/ui/base/InventoryTags.vue";
import {
    IconCalendar,
    IconPlus,
    IconRefresh,
    IconRosetteDiscount,
    IconStar,
} from "@tabler/icons-vue";
import TagItem from "~/components/modrinth/ui/base/TagItem.vue";
import ButtonStyled from "~/components/modrinth/ui/base/ButtonStyled.vue";
import InventoryFilterControl from "~/components/modrinth/ui/inventory/InventoryFilterControl.vue";
import { capitalizeString } from "~/plugins/shorthands";

const formatRelativeTime = useRelativeTime();

const props = withDefaults(
    defineProps<{
        baseId?: string;
        project: any;
        inventory: any[];
        currentMember?: any | boolean;
        itemLink?: (item: any) => string;
        openModal?: () => void;
        createItemButtonSecondary?: boolean;
    }>(),
    {
        baseId: undefined,
        currentMember: false,
        itemLink: undefined,
    },
);
const currentPage: Ref<number> = ref(1);
const pageSize: Ref<number> = ref(1);
const itemFilters: Ref<InstanceType<typeof InventoryFilterControl> | null> =
    ref(null);

const selectedStatus: Ref<string[]> = computed(
    () => itemFilters.value?.selectedStatus || [],
);
const selectedAvailability: Ref<string[]> = computed(
    () => itemFilters.value?.selectedAvailability || [],
);
const searchQuery = computed(() =>
    (itemFilters.value?.searchQuery || "").trim().toLowerCase(),
);

const filteredInventory = computed(() => {
    const needle = searchQuery.value;

    return props.inventory.filter((item) => {
        const matchesStatus = hasAnySelected(item.status, selectedStatus.value);
        const matchesAvailability = hasAnySelected(
            item.availability,
            selectedAvailability.value,
        );

        if (!needle) {
            return matchesStatus && matchesAvailability;
        }

        const searchFields = [
            item?.product?.custom_name,
            item?.product?.item?.label,
            item?.product?.item?.id,
            item?.id,
        ]
            .filter(Boolean)
            .map((value) => String(value).toLowerCase());

        const matchesSearch = searchFields.some((value) =>
            value.includes(needle),
        );

        return matchesStatus && matchesAvailability && matchesSearch;
    });
});

function hasAnySelected(values: string[], selected: string[]) {
    return (
        selected.length === 0 ||
        selected.some((value) => values.includes(value))
    );
}

function isAnySelected(value: string, selected: string[]) {
    return selected.length === 0 || selected.includes(value);
}

const currentInventory = computed(() =>
    filteredInventory.value.slice(
        (currentPage.value - 1) * pageSize.value,
        currentPage.value * pageSize.value,
    ),
);

const route = useRoute();
const router = useRouter();

if (route.query.page) {
    currentPage.value = Number(route.query.page) || 1;
}
function switchPage(page: number) {
    currentPage.value = page;

    router.replace({
        query: {
            ...route.query,
            page: currentPage.value !== 1 ? currentPage.value : undefined,
        },
    });

    window.scrollTo({ top: 0, behavior: "smooth" });
}

function updateQuery(
    newQueries: Record<string, string | string[] | undefined | null>,
) {
    if (newQueries.page) {
        currentPage.value = Number(newQueries.page);
    } else if (newQueries.page === undefined) {
        currentPage.value = 1;
    }

    router.replace({
        query: {
            ...route.query,
            ...newQueries,
        },
    });
}
</script>

<style scoped>
.versions-grid-row {
    @apply grid grid-cols-[1fr_min-content] gap-4 supports-[grid-template-columns:subgrid]:col-span-full supports-[grid-template-columns:subgrid]:!grid-cols-subgrid sm:grid-cols-[min-content_1fr_1fr_1fr_min-content];
}

.has-environment .versions-grid-row {
    @apply xl:grid-cols-[min-content_1fr_1fr_1fr_1fr_1fr_1fr_min-content];
}

.no-environment .versions-grid-row {
    @apply xl:grid-cols-[min-content_1fr_1fr_1fr_1fr_1fr_min-content];
}
</style>
