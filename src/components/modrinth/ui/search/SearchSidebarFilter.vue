    ...
    >
      <div class="flex gap-1 flex-wrap">
        ...
        <div
          ...
          class="flex gap-1 text-xs bg-button-bg px-2 py-0.5 rounded-full font-bold text-secondary w-fit shrink-0 items-center"
        >
          <IconBan class="text-brand-red" /> {{ option.formatted_name ?? option.id }}
        </div>
      </div>
    </template>
      ...
      <div class="flex flex-col gap-2 p-3 border-dashed border-2 rounded-2xl border-divider mx-2">
        <p class="m-0 font-bold items-center">
          <slot :name="`locked-${filterType.id}`">
            {{filterType.formatted_name}} is locked
          </slot>
        </p>
...
import Accordion from '~/components/modrinth/ui/base/Accordion.vue'
import type { FilterOption, FilterType, FilterValue } from '~/utils/search'

import { IconSearch, IconRefresh, IconX, IconBan, IconLockOpen2, IconChevronDown } from '@tabler/icons-vue'
...
import SearchFilterOption from '~/components/modrinth/ui/search/SearchFilterOption.vue'
