export const useUser = async (force = false) => {
    ...
    if (
        !user.value ||
        force ||
        (user.value && Date.now() - user.value.lastUpdated > 300000)
    ) {
    ...
    if (auth.user && auth.user.id) {
        try {
            ...
            const [follows, collections] = await Promise.all([
                ...
                useBaseFetch(
                    `user/${auth.user.id}/collections`,
                    { apiVersion: 3, headers },
                    true
                ),
            ]);
...
export const initUserCollections = async () => {
    ...
    if (auth.user && auth.user.id) {
        try {
            user.collections = await useBaseFetch(
                `user/${auth.user.id}/collections`,
                { apiVersion: 3 }
            );
        } catch (err) {
...
export const userCollectProject = async (collection, projectId) => {
    ...
    const latestCollection = user.collections.find(
        (x) => x.id === collectionId
    );
    if (!latestCollection) {
