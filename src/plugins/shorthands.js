...
export default defineNuxtPlugin((nuxtApp) => {
    const tagStore = useTags(); // change back to this when fixed
    // const tagStore = "";
    ...
    nuxtApp.provide("formatVersion", (versionsArray) =>
        formatVersions(tagStore, versionsArray)
    );
  ...
  */
    nuxtApp.provide("computeVersions", (versions, members) => {
        ...
        for (const version of versions.sort(
            (a, b) =>
                nuxtApp.$dayjs(a.date_published) -
                nuxtApp.$dayjs(b.date_published)
        )) {
    ...
    nuxtApp.provide("getProjectTypeForDisplay", (type, categories) => {
        if (type === "mod") {
            const isPlugin = categories.some((category) => {
                return tagStore.value.loaderData.allPluginLoaders.includes(
                    category
                );
            });
            ...
            const isDataPack = categories.some((category) => {
                return tagStore.value.loaderData.dataPackLoaders.includes(
                    category
                );
            });
