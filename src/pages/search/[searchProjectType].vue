<template>
    ...
    >
        <Head>
            <Title>Search {{ projectType.display }}s - Vanilla Tyme</Title>
        </Head>
        <Teleport
            v-if="flags.searchBackground"
          ...
          />
                ...
                >
                    ...
                    <template #locked-game_version>
                        Game version is provided by the server
                    </template>
                    <template #locked-mod_loader>
                        Loader is provided by the server
                    </template>
                    <template #sync-button> Sync with server </template>
                </SearchSidebarFilter>
            </div>
        </aside>
            ...
            <div class="flex flex-col gap-3">
                ...
                <SearchFilterControl
                    ...
                    :overridden-provided-filter-types="
                        overriddenProvidedFilterTypes
                    "
                    provided-message="Provided by the server"
                />
                <ClientOnly v-if="searchLoading && !noLoad">
                    <IceCreamLoader />
                </ClientOnly>
                <div
                    v-else-if="
                    ...
                    >
                        ...
                        >
                            <ProjectCard
                                ...
                                "
                                :featured-image="
                                    // result.featured_gallery ? result.featured_gallery : result.gallery[0] //original code
                                ...
                                :search="true"
                                :show-updated-date="
                                    !server && currentSortType.name !== 'newest'
                                "
                                :show-created-date="!server"
                                :color="result.color"
                                :stall_color="result.stall_color"
...
import SearchSidebarFilter from "~/components/modrinth/ui/search/SearchSidebarFilter.vue";
...
import NewProjectCard from "~/components/modrinth/ui/project/NewProjectCard.vue";
import SearchFilterControl from "~/components/modrinth/ui/search/SearchFilterControl.vue";
import { computed } from "vue";
import ProjectCard from "~/components/ui/ProjectCard.vue";
// import LogoAnimated from "~/components/brand/LogoAnimated.vue";
// import AdPlaceholder from "~/components/ui/AdPlaceholder.vue";
import NavTabs from "~/components/ui/NavTabs.vue";
import IceCreamLoader from "~/components/ui/IceCreamLoader.vue";
// IconGame find alt
import {
    IconCheck,
    IconDownload,
...
async function updateServerContext() {
    if (
        route.query.sid &&
        (!server.value || server.value.serverId !== route.query.sid)
    ) {
        if (!auth.value.user) {
            router.push(
                "/auth/sign-in?redirect=" + encodeURIComponent(route.fullPath),
            );
        } else if (route.query.sid !== null) {
            server.value = await usePyroServer(route.query.sid, [
                "general",
                "content",
            ]);
        }
...
async function serverInstall(project) {
    ...
    try {
        ...
        const version =
            versions.find(
                (x) =>
                    ...
                    x.loaders.includes(
                        server.value.general.loader.toLowerCase(),
                    ),
            ) ?? versions[0];
        ...
        if (projectType.value.id === "modpack") {
            ...
            await server.value.content.install(
                "mod",
                version.project_id,
                version.id,
            );
...
const pageCount = computed(() =>
    results.value
        ? Math.ceil(results.value.total_hits / results.value.limit)
        : 1,
);
