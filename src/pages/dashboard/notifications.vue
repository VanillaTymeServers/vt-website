...
import { IconHistory, IconChecks } from "@tabler/icons-vue";
import {
...
const { data, pending, error, refresh } = await useAsyncData(
    async () => {
        ...
        const notifications = await useBaseFetch(
            `user/${auth.value.user.id}/notifications`,
        );
        ...
        return fetchExtraNotificationData(
            filteredNotifications.slice(
                pageNum * perPage.value,
                perPage.value + pageNum * perPage.value,
            ),
        ).then((notifications) => {
            return {
                notifications,
...
async function readAll() {
    const ids = notifications.value.flatMap((notification) => [
        ...
        ...(notification.grouped_notifs
            ? notification.grouped_notifs.map((notif) => notif.id)
            : []),
    ]);
