...
import {
    IconDeviceDesktop as ClientIcon,
    IconBuildingStore,
    IconShoppingCart,
...
export type FilterOption = BaseOption &
    (
        ...
        | { method: "environment"; environment: "stall" | "shop" }
    );
...
export function useSearch(
    ...
    const filters = computed(() => {
        ...
        for (const category of sortByNameOrNumber(
            tags.value.categories.slice(),
            ["header", "name"],
        )) {
        ...
        const filterTypes: FilterType[] = [
            {
                id: "type",
                formatted_name: "Types",
                supported_project_types: ["shop"],
                query_param: "t",
                supports_negative_filter: true,
                display: "all",
                searchable: false,
    ...
    const facets = computed(() => {
        const validProvidedFilters = providedFilters.value.filter(
            (providedFilter) =>
                !overriddenProvidedFilterTypes.value.includes(
                    providedFilter.type,
                ),
        );
        ...
        for (const filterValue of filterValues) {
            const type = filters.value.find(
                (type) => type.id === filterValue.type,
            );
            if (!type) {
     ...
     */
        const client = currentFilters.value.some(
            (filter) =>
                filter.type === "environment" && filter.option === "client",
        );
        const server = currentFilters.value.some(
            (filter) =>
                filter.type === "environment" && filter.option === "server",
        );
    ...
    const requestParams: Ref<string> = computed(() => {
        ...
        params.push(
            `facets=${encodeURIComponent(JSON.stringify(facets.value))}`,
        );
    ...
    function readQueryParams() {
        ...
        loadQueryParam(["s"], (sort) => {
            currentSortType.value =
                sortTypes.find((sortType) => sortType.name === sort) ??
                sortTypes[0];
            readParams.add("s");
        });
        ...
        loadQueryParam(["o"], (offset) => {
            currentPage.value =
                Math.ceil(Number(offset) / maxResults.value) + 1;
            readParams.add("o");
        });
...
function getOptionValue(option: FilterOption, negative?: boolean): string {
    let value =
        option.method === "or" || option.method === "and"
            ? option.value
            : option.id;
    if (negative === true) {
        value = value.replace(":", "!=");
